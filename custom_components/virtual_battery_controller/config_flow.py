...
async def get_options_schema(handler):
    ...
    return vol.Schema(
        {
            vol.Required(
                CONF_PRICE_SENSOR, default=data.get(CONF_PRICE_SENSOR) or ""
            ): selector.EntitySelector(selector.EntitySelectorConfig(domain="sensor")),
            vol.Required(
                CONF_CLIMATE_ENTITY, default=data.get(CONF_CLIMATE_ENTITY) or ""
            ): selector.EntitySelector(selector.EntitySelectorConfig(domain="climate")),
            vol.Optional(
                CONF_SWITCH_ENTITIES, default=data.get(CONF_SWITCH_ENTITIES) or []
            ...
            vol.Optional(
                CONF_SOLAR_EXPORT_SENSOR,
                default=data.get(CONF_SOLAR_EXPORT_SENSOR) or "",
            ): selector.EntitySelector(selector.EntitySelectorConfig(domain="sensor")),
            vol.Required(
                CONF_HVAC_MODE, default=data.get(CONF_HVAC_MODE) or MODE_HEATING
            ): vol.In([MODE_HEATING, MODE_COOLING]),
            vol.Required(
                CONF_AIRCO_WATTAGE, default=data.get(CONF_AIRCO_WATTAGE) or 1000
            ): int,
...
OPTIONS_FLOW = {"init": SchemaFlowFormStep(get_options_schema)}
            ...
            data_schema=vol.Schema(
                {
                    ...
                    vol.Optional(CONF_SWITCH_ENTITIES): selector.EntitySelector(
                        selector.EntitySelectorConfig(
                            domain=["switch", "input_boolean"], multiple=True
                        )
                    ),
                    ...
                    vol.Required(CONF_HVAC_MODE, default=MODE_HEATING): vol.In(
                        [MODE_HEATING, MODE_COOLING]
                    ),
